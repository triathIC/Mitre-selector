...
- DetailPanel close (the **✕ button and the Escape key**) now returns to the matrix (`/matrix`) instead of the Scenario Library (`/`). Both handlers still called `navigate("/")`, a stale assumption from before the scenario-layer migration moved the matrix off `/`; the technique you were inspecting lives on the matrix, so closing the panel should land you back there.
- Navigation no longer highlights **"Matrix" and "KQL Library" at the same time**. "KQL Library" is not its own route — it is `/matrix?hasKql=true`, sharing a pathname with "Matrix". `NavLink` compares pathnames only and ignored the query string, so both items matched on every matrix route (and `end` could not tell them apart). The `Header` now computes the active item from `location.search`, so exactly one nav item is highlighted at any time, with `aria-current="page"` set on just that item.
