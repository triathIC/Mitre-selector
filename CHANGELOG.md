...
- **Scenario `clickfix-potemkin`** (`src/data/scenarios/scenario-clickfix-potemkin.json`): new static-reviewed scenario at `/scenario/clickfix-potemkin` — "ClickFix → Potemkin Loader → Hands-on-Keyboard" (Huntress, May 2026; corroborated by CyberSecurityNews). 11 kill-chain steps across 13 ATT&CK v19 techniques, a hero correlation block ("detection isn't a query, it's a chain") and five thesis blind spots (unmonitored patient-zero endpoint, blockchain/EtherHiding C2, reflective in-memory loading, renamed cloudflared, in-memory AMSI patch). ATT&CK v19 remap applied: the Defender-teardown step uses `T1685` (Disable or Modify Tools, tactic Defense Impairment) — the legacy `T1562.001` no longer exists in the pinned v19.1 bundle; the Mshta/Msiexec/Reflective-Code-Loading steps resolve to the `Stealth` tactic.
- **Four KQL detections** in `public/data/kql_mappings.json` (mirrored to `data/kql_mappings.json`) backing the scenario's hero steps, so their `detection.kqlId` resolves to real `mapping_id`s instead of falling back to `kqlDraft`: `KQL-T1204.004-001` (ClickFix chain correlation: proxy exec + AppData persistence + Defender tamper, joined on one DeviceId within 30m), `KQL-T1218.005-002` (pcalua proxying mshta/msiexec), `KQL-T1547.001-003` (Run-key → AppData/ProgramData executable, hunting), `KQL-T1685-003` (Defender teardown: Set-MpPreference / exclusions / service stop). All `confidence: experimental` (static-reviewed, untested), tagged with `stealth`/`defense-impairment` (never the CI-blocked `defense-evasion`); mapping count 90 → 94, `check-mitre-bundle` green.
- **`@triathic/mke-core` 0.1.0** (`packages/mke-core`): the framework-agnostic core extracted into a publishable workspace package — ATT&CK v19 types and tactic surface (`MitreTechnique`, `MitreTactic`, `TACTIC_ORDER`, `PLATFORMS`, …), `KqlMapping` schema types, scenario-layer types (`Scenario`, `KillChainStep`, `MaturityLevel`, …, moved from `src/types/scenario.ts`), pure data transforms (`buildDataStore`, `loadDataStore`) and the contribution-URL helper (all moved from `src/core/`). New in the package (previously nowhere in the repo): technique-ID format validation (`TECHNIQUE_ID_PATTERN`, `isTechniqueId`) and a dependency-free `validateKqlMapping` schema validator. Built with plain `tsc` (ESM + `.d.ts` into `dist/`, `strict` + `noUncheckedIndexedAccess`); published to GitHub Packages (`publishConfig.registry`). Data/content JSONs and their maintenance scripts intentionally stay app-side.
- `.github/workflows/publish.yml` — publishes `@triathic/mke-core` to GitHub Packages on `mke-core-v*` tag pushes (e.g. the `mke-core-v0.1.0` release). The package-scoped tag namespace deliberately avoids the app's existing `v*` release tags (`v0.2.0`), so app releases never re-trigger a publish. Auth via the workflow's own `GITHUB_TOKEN` (`packages: write`); `npm ci` + workspace build + `npm publish --workspace packages/mke-core`.
- Root `package.json` now declares the repo's existing `license: MIT` explicitly (the field was missing); `packages/mke-core` is MIT too and ships a copy of the root `LICENSE` so npm includes it in the tarball. (The build brief proposed Apache-2.0, but the repo has been MIT since day one — owner decided to keep everything MIT.)
...
- Cold-call deep links (e.g. `https://mitre.triath.xyz/scenario/clickfix-potemkin`, `/matrix`, `/technique/T1055`) returned 404 from Vercel while in-app navigation worked. Root cause was **not** a missing SPA rewrite: `vite-react-ssg` pre-renders every route as a flat HTML file (`dist/scenario/clickfix-potemkin.html`, `dist/matrix.html`, one file per technique), but without a `vercel.json` Vercel only served them under their `.html` paths — the extensionless URLs (which the sitemap declares as canonical) 404ed. New `vercel.json` with `"cleanUrls": true` serves each pre-rendered page under its extensionless URL (and 308-redirects the `.html` variants onto it) plus `"trailingSlash": false` to normalize `/matrix/` → `/matrix`. Deliberately **no** rewrites to `/index.html`: that would ship the Scenario-Library shell (wrong `<title>`/meta/JSON-LD) for every deep link, defeating the SSG/SEO setup, and would mask genuine 404s for invalid technique IDs; with `cleanUrls` alone, `/data/*.json`, `/assets/*`, `/static-loader-data/*`, `sitemap.xml`, `robots.txt` and the SVGs are untouched by construction.
- `Header.tsx` imported `@/core/models` again after the `main` → `feat/mke-core-extraction` merge: the nav-highlight rewrite from PR #9 (newer on `main`) won the merge for the whole file, including its pre-extraction import line — which broke the Vercel deploy of PR #11 (`TS2307`). Re-pointed the import to `@triathic/mke-core`; the nav-fix body from PR #9 is kept unchanged.
...
- **Workspace layout**: the repo is now an npm-workspaces monorepo (`workspaces: ["packages/*"]`); the app stays at the repo root and consumes `@triathic/mke-core` as a workspace dependency. All 19 app files that imported `@/core/*` or `@/types/scenario` now import from `@triathic/mke-core`; `src/core/` and `src/types/` are gone. The package's `prepare` script builds it on every `npm install`/`npm ci`, so cold clones, CI and Vercel always have `dist/` before app builds or typed lint run (a single fresh `tsc -b` can't resolve just-emitted output through the workspace symlink in the same process); the root `tsconfig.json` additionally references the package so incremental `tsc -b` keeps it fresh, and a `predev` script rebuilds it before `npm run dev`. ESLint's `parserOptions.project` covers the package tsconfig; `packages/*/dist` added to ESLint ignores. The `security.yml` eslint-security job lints `packages/mke-core/src/` alongside `src/`, and the secret-scan job scans `packages/` too, so the extracted code keeps its CI security coverage. CONTRIBUTING.md `KqlMapping` link and the mitre-drift issue template updated to the new paths.
